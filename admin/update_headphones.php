...
if(isset($_POST['update'])){
   ...
   $update_product = $conn->prepare("UPDATE `headphones` SET name = ?, price = ?, details = ? WHERE id = ?");
   ...
   if(!empty($image_01)){
      if($image_size_01 > 2000000){
         ...
         $update_image_01 = $conn->prepare("UPDATE `headphones` SET image_01 = ? WHERE id = ?");
   ...
   if(!empty($image_02)){
      if($image_size_02 > 2000000){
         ...
         $update_image_02 = $conn->prepare("UPDATE `headphones` SET image_02 = ? WHERE id = ?");
   ...
   if(!empty($image_03)){
      if($image_size_03 > 2000000){
         ...
         $update_image_03 = $conn->prepare("UPDATE `headphones` SET image_03 = ? WHERE id = ?");
...
<section class="update-product">
   ...
   <h1 class="heading">Обновете на слушалки</h1>

   <?php
      $update_id = $_GET['update'];
 
      $select_products = $conn->prepare("SELECT * FROM `headphones` WHERE id = ?");
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <span>Обновете изображението 03</span>
      <input type="file" name="image_03" accept="image/jpg, image/jpeg, image/png, image/webp" class="box">
      <div class="flex-btn">
         <input type="submit" name="update" class="btn" value="Обновете">
         <a href="headphones.php" class="option-btn">Върнете се обратно</a>
      </div>
   </form>
