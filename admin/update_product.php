...
if(isset($_POST['update'])){
   ...
   $update_product = $conn->prepare("UPDATE `products` SET name = ?, price = ?, details = ? WHERE id = ?"); // Променя информацията за определен продукт със зададен идентификатор (id). Приемат се параметри с новото име (name), цена (price) и детайли (details) на продукта;
   $update_product->execute([$name, $price, $details, $pid]); // Изпълнява заявката с масив от стойности, които да бъдат вмъкнати; 
...
<section class="update-product">
   ...
   <h1 class="heading">Обновяване на продукта</h1>
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <div class="flex-btn">
         ...
         <a href="products.php" class="option-btn">Върнете се обратно</a>
      </div>
   </form>
