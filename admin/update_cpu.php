...
if(isset($_POST['update'])){
   ...
   $update_product = $conn->prepare("UPDATE `cpu` SET name = ?, price = ?, details = ? WHERE id = ?");
   ...
   if(!empty($image_01)){
      if($image_size_01 > 2000000){
         ...
         $update_image_01 = $conn->prepare("UPDATE `cpu` SET image_01 = ? WHERE id = ?");
   ...
   if(!empty($image_02)){
      if($image_size_02 > 2000000){
         ...
         $update_image_02 = $conn->prepare("UPDATE `cpu` SET image_02 = ? WHERE id = ?");
   ...
   if(!empty($image_03)){
      if($image_size_03 > 2000000){
         ...
         $update_image_03 = $conn->prepare("UPDATE `cpu` SET image_03 = ? WHERE id = ?");
...
<head>
   ...
   <title>Обновяване не продукта</title>
...
<section class="update-product">
   ...
   <h1 class="heading">Обновяване на процесори</h1>

   <?php
      $update_id = $_GET['update'];
      $select_products = $conn->prepare("SELECT * FROM `cpu` WHERE id = ?");
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <div class="flex-btn">
         ...
         <a href="CPU.php" class="option-btn">Върнете се обратно</a>
      </div>
   </form>
