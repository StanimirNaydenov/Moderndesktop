...
if(isset($_POST['add_product'])){
   ...
   $select_products = $conn->prepare("SELECT * FROM `ssd` WHERE name = ?");
   ...
   if($select_products->rowCount() > 0){
      ...
      $insert_products = $conn->prepare("INSERT INTO `ssd`(name, details, price, image_01, image_02, image_03) VALUES(?,?,?,?,?,?)");
...
if(isset($_GET['delete'])){
   ...
   $delete_product_image = $conn->prepare("SELECT * FROM `ssd` WHERE id = ?");
   ...
   $delete_product = $conn->prepare("DELETE FROM `ssd` WHERE id = ?");
   $delete_product->execute([$delete_id]);
   $delete_cart = $conn->prepare("DELETE FROM `cart` WHERE pid = ?");
   $delete_cart->execute([$delete_id]);
   $delete_wishlist = $conn->prepare("DELETE FROM `wishlist` WHERE pid = ?");
   $delete_wishlist->execute([$delete_id]);
   header('location:sdd.php');
}
...
<section class="add-products">
   ...
   <h1 class="heading"> Добавете ssd-та</h1>
   ...
   <form action="" method="post" enctype="multipart/form-data">
      <div class="flex">
        ...
        <div class="inputBox">
         ...
         <div class="inputBox">
            <span>Подробности за продукта (изисква)</span>
...
<section class="show-products">
   ...
   <div class="box">
      ...
      <div class="flex-btn">
         <a href="update_sdd.php?update=<?= $fetch_products['id']; ?>" class="option-btn">Обновете</a>
         <a href="sdd.php?delete=<?= $fetch_products['id']; ?>" class="delete-btn" onclick="return confirm('delete this product?');">Изтрийте</a>
      </div>
   </div>
   <?php
         }
