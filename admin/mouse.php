...
if(isset($_POST['add_product'])){
   ...
   $select_products = $conn->prepare("SELECT * FROM `mouse` WHERE name = ?");
   ...
   if($select_products->rowCount() > 0){
      ...
      $insert_products = $conn->prepare("INSERT INTO `mouse`(name, details, price, image_01, image_02, image_03) VALUES(?,?,?,?,?,?)");
...
if(isset($_GET['delete'])){
   ...
   $delete_product_image = $conn->prepare("SELECT * FROM `mouse` WHERE id = ?");
   ...
   $delete_product = $conn->prepare("DELETE FROM `mouse` WHERE id = ?");
...
<section class="add-products">
   ...
   <h1 class="heading">Добавете мишки</h1>
...
<section class="show-products">
   ...
   <div class="box">
      ...
      <div class="flex-btn">
         <a href="update_mouse.php?update=<?= $fetch_products['id']; ?>" class="option-btn">Обновете</a>
         <a href="mouse.php?delete=<?= $fetch_products['id']; ?>" class="delete-btn" onclick="return confirm('delete this product?');">Изтрийте</a>
      </div>
   </div>
   <?php
         }
