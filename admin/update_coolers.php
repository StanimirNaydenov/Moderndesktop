...
if(isset($_POST['update'])){
   ...
   $update_product = $conn->prepare("UPDATE `coolers` SET name = ?, price = ?, details = ? WHERE id = ?");
   ...
   if(!empty($image_01)){
      if($image_size_01 > 2000000){
         ...
         $update_image_01 = $conn->prepare("UPDATE `coolers` SET image_01 = ? WHERE id = ?");
   ...
   if(!empty($image_02)){
      if($image_size_02 > 2000000){
         ...
         $update_image_02 = $conn->prepare("UPDATE `coolers` SET image_02 = ? WHERE id = ?");
   ...
   if(!empty($image_03)){
      if($image_size_03 > 2000000){
         ...
         $update_image_03 = $conn->prepare("UPDATE `coolers` SET image_03 = ? WHERE id = ?");
...
<head>
   ...
   <title>Обновете на продукта</title>
...
<section class="update-product">
   ...
   <h1 class="heading">ОБНОВЯВАНЕ на охладители</h1>

   <?php
      $update_id = $_GET['update'];
 
      $select_products = $conn->prepare("SELECT * FROM `coolers` WHERE id = ?");
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <span>Обновяване на изображениe 01</span>
      <input type="file" name="image_01" accept="image/jpg, image/jpeg, image/png, image/webp" class="box">
      <span>Обновяване на изображениe 02</span>
      ...
      <div class="flex-btn">
         ...
         <a href="coolers.php" class="option-btn">Върнете се обратно</a>
      </div>
   </form>
