...
if(isset($_POST['update'])){
   ...
   $update_product = $conn->prepare("UPDATE `hdd` SET name = ?, price = ?, details = ? WHERE id = ?");
   ...
   if(!empty($image_01)){
      if($image_size_01 > 2000000){
         ...
         $update_image_01 = $conn->prepare("UPDATE `hdd` SET image_01 = ? WHERE id = ?");
   ...
   if(!empty($image_02)){
      if($image_size_02 > 2000000){
         ...
         $update_image_02 = $conn->prepare("UPDATE `hdd` SET image_02 = ? WHERE id = ?");
   ...
   if(!empty($image_03)){
      if($image_size_03 > 2000000){
         ...
         $update_image_03 = $conn->prepare("UPDATE `hdd` SET image_03 = ? WHERE id = ?");
...
<section class="update-product">
   ...
   <h1 class="heading">Обновяване на твърди дискове</h1>

   <?php
      $update_id = $_GET['update'];
 
      $select_products = $conn->prepare("SELECT * FROM `hdd` WHERE id = ?");
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <input type="text" name="name" required class="box" maxlength="100" placeholder="Въведеет име на продукта" value="<?= $fetch_products['name']; ?>">
      ...
      <div class="flex-btn">
         ...
         <a href="hdd.php" class="option-btn">Върнете се обратно</a>
      </div>
   </form>
