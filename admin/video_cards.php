...
if(isset($_POST['add_product'])){
   ...
   $select_products = $conn->prepare("SELECT * FROM `videocards` WHERE name = ?");
   ...
   if($select_products->rowCount() > 0){
      ...
      $insert_products = $conn->prepare("INSERT INTO `videocards`(name, details, price, image_01, image_02, image_03) VALUES(?,?,?,?,?,?)");
...
if(isset($_GET['delete'])){
   ...
   $delete_product_image = $conn->prepare("SELECT * FROM `videocards` WHERE id = ?");
   ...
   $delete_product = $conn->prepare("DELETE FROM `videocards` WHERE id = ?");
...
<section class="add-products">
   ...
   <h1 class="heading">Добавете видео карти</h1>
...
<section class="show-products">
   ...
   <div class="box">
      ...
      <div class="flex-btn">
         <a href="update_gpu.php?update=<?= $fetch_products['id']; ?>" class="option-btn">Обновете</a>
         <a href="videocards.php?delete=<?= $fetch_products['id']; ?>" class="delete-btn" onclick="return confirm('delete this product?');">Изтрийте</a>
      </div>
   </div>
   <?php
         }
