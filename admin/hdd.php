...
if(isset($_POST['add_product'])){
   ...
   $select_products = $conn->prepare("SELECT * FROM `hdd` WHERE name = ?");
   ...
   if($select_products->rowCount() > 0){
      ...
      $insert_products = $conn->prepare("INSERT INTO `hdd`(name, details, price, image_01, image_02, image_03) VALUES(?,?,?,?,?,?)");
...
if(isset($_GET['delete'])){
   ...
   $delete_product_image = $conn->prepare("SELECT * FROM `hdd` WHERE id = ?");
   ...
   $delete_product = $conn->prepare("DELETE FROM `hdd` WHERE id = ?");
...
<section class="add-products">
   ...
   <h1 class="heading">Добавете твърди дискове</h1>
      ...
      <div class="flex">
        ...
        <div class="inputBox">
         ...
         <div class="inputBox">
            ...
            <textarea name="details" placeholder="Въвдете подробности за продукта" class="box" required maxlength="500" cols="30" rows="10"></textarea>
         </div>
      </div>
...
<section class="show-products">
   ...
   <div class="box">
      ...
      <div class="flex-btn">
         <a href="update_hdd.php?update=<?= $fetch_products['id']; ?>" class="option-btn">Обновете</a>
         <a href="hdd.php?delete=<?= $fetch_products['id']; ?>" class="delete-btn" onclick="return confirm('delete this product?');">Изтрийте</a>
      </div>
   </div>
   <?php
         }
      }else{
         echo '<p class="empty">Няма добавени продукти ощи!</p>';
      }
   ?>
