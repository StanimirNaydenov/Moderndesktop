...
if(isset($_POST['add_product'])){
   ...
   $select_products = $conn->prepare("SELECT * FROM `ram` WHERE name = ?");
   ...
   if($select_products->rowCount() > 0){
      ...
      $insert_products = $conn->prepare("INSERT INTO `ram`(name, details, price, image_01, image_02, image_03) VALUES(?,?,?,?,?,?)");
...
if(isset($_GET['delete'])){
   ...
   $delete_product_image = $conn->prepare("SELECT * FROM `ram` WHERE id = ?");
   ...
   $delete_product = $conn->prepare("DELETE FROM `ram` WHERE id = ?");
...
<section class="add-products">
   ...
   <h1 class="heading">Добавете ram</h1>
   ...
   <form action="" method="post" enctype="multipart/form-data">
      <div class="flex">
         ...
         <div class="inputBox">
            ...
            <input type="number" min="0" class="box" required max="9999999999" placeholder="Въведет цена на продукта" onkeypress="if(this.value.length == 10) return false;" name="price">
         </div>
        <div class="inputBox">
        ...
        <div class="inputBox">
         ...
         <div class="inputBox">
            ...
            <textarea name="details" placeholder="Въведете подробности за продукти" class="box" required maxlength="500" cols="30" rows="10"></textarea>
         </div>
      </div>
...
<section class="show-products">
   ...
   <div class="box">
      ...
      <div class="flex-btn">
         <a href="update_ram.php?update=<?= $fetch_products['id']; ?>" class="option-btn">Обновете</a>
         <a href="ram.php?delete=<?= $fetch_products['id']; ?>" class="delete-btn" onclick="return confirm('delete this product?');">Изтрийте</a>
      </div>
   </div>
   <?php
         }
